...
export function AudioImportControl({ isImporting, disabled = false, onSelectFile }: Props) {
  return <div className="import-control">
    <label className="file-cta" aria-busy={isImporting} title={isImporting ? '正在转写音频' : '添加音频'}>
      ...
      <input aria-label="选择音频文件" type="file" accept="audio/*" disabled={isImporting || disabled} onChange={(event) => {
