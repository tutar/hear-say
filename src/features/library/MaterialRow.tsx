...
export function MaterialRow({ material, actions, onOpen, onRename, onDelete, onToggleFavorite, availableTags = [], onSaveTags, onExport, onResetProgress, onManageSubtitles }: Props) {
  ...
  return <li className="material-row material-card" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setMenu(false) }}>
    ...
    {resetting && <div className="dialog-backdrop"><section className="delete-dialog" role="dialog" aria-modal="true"><p className="eyebrow">Learning progress</p><h3>重置学习进度？</h3><p>首次学习将重新开始，当前复习计划与未完成练习会被删除，复习计划会重新生成。</p><p>保留音频、字幕、标签和收藏，也会保留历史学习时间、收藏难句、随心听位置和全局偏好。</p><div className="dialog-actions"><button className="dialog-secondary" onClick={() => setResetting(false)}>取消重置</button><button className="dialog-primary" onClick={() => void onResetProgress?.(material.id)}>确认重置</button></div></section></div>}
    {deleting && <div className="dialog-backdrop"><section className="delete-dialog" role="dialog" aria-modal="true"><p className="eyebrow">Delete material</p><h3>删除“{material.title}”吗？</h3><p>音频、句子时间轴与学习记录将一并永久删除。</p><div className="dialog-actions"><button className="dialog-secondary" onClick={() => setDeleting(false)}>取消删除</button><button className="dialog-danger" onClick={() => void onDelete(material.id)}>确认删除</button></div></section></div>}
  </li>
}
