...
import { SentenceSupportPanel } from './SentenceSupportPanel'
...
export function FreeListening({ material, segments, preferences: initialPreferences, progress, onPreferencesChange, onProgressChange, onPlaybackChange, mode = 'free', completionAction, onAudioEnded }: Props) {
  ...
  return <main className="free-listening">
    ...
    <section className={`free-listening-stage ${preferences.viewMode === 'list' ? 'is-list' : 'is-single'} ${preferences.textVisible ? '' : 'is-hidden'}`} aria-label="播放内容">
      ...
      </div> : <><ol className="free-sentence-list">{segments.map((segment, index) => <li key={segment.id}><button ref={(node) => { rowRefs.current[index] = node }} type="button" aria-current={index === segmentIndex ? 'true' : undefined} onClick={() => { selectSegment(index); setDetailIndex(index) }}><span>{String(index + 1).padStart(2, '0')}</span>{preferences.textVisible ? segment.text : <i>{maskText(segment.text)}</i>}</button></li>)}</ol>{detailIndex !== null && <SentenceSupportPanel sentence={segments[detailIndex]?.text ?? ''} state={{ analysis: preferences.analysisVisible, translation: preferences.translationVisible, chunks: preferences.chunksVisible }} onChange={(state) => changePreferences({ analysisVisible: state.analysis, translationVisible: state.translation, chunksVisible: state.chunks })} onClose={() => setDetailIndex(null)} />}</>}
    </section>
