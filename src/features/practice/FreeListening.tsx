...
export function FreeListening({ material, segments, preferences: initialPreferences, progress, onPreferencesChange, onProgressChange, onPlaybackChange }: Props) {
  const [preferences, setPreferences] = useState(initialPreferences)
  const [segmentIndex, setSegmentIndex] = useState(Math.min(progress?.segmentIndex ?? 0, Math.max(segments.length - 1, 0)))
  const [position, setPosition] = useState(progress?.positionSeconds ?? segments[segmentIndex]?.startSeconds ?? 0)
  ...
  function selectSegment(index: number, autoplay = playing) {
    ...
    setPosition(segment.startSeconds)
    if (audioRef.current) {
  ...
  return <main className="free-listening">
    <audio ref={audioRef} src={audioUrl} onTimeUpdate={onTimeUpdate} onPlay={() => { setPlaying(true); onPlaybackChange?.(true) }} onPause={() => { setPlaying(false); saveProgress(); onPlaybackChange?.(false) }} />
    <header className="free-listening-heading"><span className="listening-mode">随心听</span><div><p className="eyebrow">Free listening</p><h1>{material.title}</h1></div><PlaybackRateSelect value={preferences.playbackRate} onChange={(playbackRate) => changePreferences({ playbackRate })} compact /></header>
    <section className={`free-listening-stage ${preferences.textVisible ? '' : 'is-hidden'}`} aria-label="播放内容">
      ...
      </div> : <ol className="free-sentence-list">{segments.map((segment, index) => <li key={segment.id}><button ref={(node) => { rowRefs.current[index] = node }} type="button" aria-current={index === segmentIndex ? 'true' : undefined} onClick={() => selectSegment(index)}><span>{String(index + 1).padStart(2, '0')}</span>{preferences.textVisible ? segment.text : <i>文本已隐藏</i>}</button></li>)}</ol>}
    </section>
    <section className="free-player" aria-label="播放器">
      <div className="free-progress"><span>{formatTime(position)}</span><div><input aria-label="播放进度" type="range" min={0} max={material.durationSeconds ?? current?.endSeconds ?? 0} step="0.1" value={position} onChange={(event) => { const value = Number(event.target.value); setPosition(value); if (audioRef.current) audioRef.current.currentTime = value }} /><i style={{ width: `${material.durationSeconds ? Math.min(100, position / material.durationSeconds * 100) : 0}%` }} /></div><span>{formatTime(material.durationSeconds ?? 0)}</span></div>
      ...
      <div className="free-options">
        ...
        <button className="eye-control" type="button" aria-label={preferences.textVisible ? '隐藏文本' : '显示文本'} onClick={() => changePreferences({ textVisible: !preferences.textVisible })}><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z" /><circle cx="12" cy="12" r="2.8" />{!preferences.textVisible && <path d="m4 4 16 16" />}</svg>{preferences.textVisible ? '隐藏文本' : '显示文本'}</button>
