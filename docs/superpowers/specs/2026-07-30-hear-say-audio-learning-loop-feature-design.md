...
材料库中的重命名采用原地编辑：保存时去除首尾空格，空名称不写入；取消不产生任何写入。删除使用项目内自定义确认弹框，不使用浏览器原生确认框。弹框固定提示“音频、句子时间轴与学习记录将一并永久删除”；确认后在同一事务中移除材料 Blob、全部句子、难句和学习状态，取消不改变数据。改名和删除都适用于失败材料，且不会改动电脑上的源文件。
